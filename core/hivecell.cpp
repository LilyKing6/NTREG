    ...
    HCELL_INDEX CellIndex)
{
    ...
    // DPRINT("%s - Hive %p, CellIndex %08lx\n", __FUNCTION__, RegistryHive, CellIndex);
    ...
    if (!RegistryHive->Flat)
    {
        ...
        // printf("CellBlock: %d\n", CellBlock);
        // printf("RegistryHive->Storage[CellType].Length: %d\n\n", RegistryHive->Storage[CellType].Length);
        ASSERT(CellBlock < RegistryHive->Storage[CellType].Length);
        Block = reinterpret_cast<void*>(RegistryHive->Storage[CellType].BlockList[CellBlock].BlockAddress);
        ASSERT(Block != NULL);
        return reinterpret_cast<PHCELL>(static_cast<char*>(Block) + CellOffset);
    }
    else
    {
        ASSERT(HvGetCellType(CellIndex) == Stable);
        return reinterpret_cast<PHCELL>(reinterpret_cast<char*>(RegistryHive->BaseBlock) + HBLOCK_SIZE +
                        CellIndex);
    }
}
    ...
    void* Cell)
{
    ...
    return (static_cast<PHCELL>(Cell) - 1)->Size;
}
              ...
              IN void* Address)
{
    ...
    CellHeader = static_cast<PHCELL>(Address) - 1;
    ...
    BOOLEAN HoldingLock)
{
     ...
     */
    /*
    *FIXME：需要查询延迟刷新操作，以确保脏数据相应地被刷新到磁盘
        但是，此操作必须在像HvMarkDirty这样的助手中完成，该助手将配置单元的特定部分标记为脏
        由于我们没有这样的助手，因此在暂时将单元格标记为脏时，我们必须执行最终的惰性刷新，这样我们不仅刷新脏数据，还写入日志
    */
    ...
    HCELL_INDEX FreeIndex)
{
    ...
    Storage = static_cast<HSTORAGE_TYPE>(HvGetCellType(FreeIndex));
    ...
    HCELL_INDEX CellIndex)
{
    ...
    Storage = static_cast<HSTORAGE_TYPE>(HvGetCellType(CellIndex));    Index = HvpComputeFreeListIndex((ULONG)CellBlock->Size);
    ...
    PHHIVE Hive)
{
    ...
    {
        Bin = reinterpret_cast<PHBIN>(Hive->Storage[Stable].BlockList[BlockIndex].BinAddress);

        /* Search free blocks and add to list */
        FreeOffset = sizeof(HBIN);
        while (FreeOffset < Bin->Size)
        {
            FreeBlock = reinterpret_cast<PHCELL>(reinterpret_cast<char*>(Bin) + FreeOffset);
            if (FreeBlock->Size > 0)
            {
    ...
    ULONG Size)
{
    ...
    Storage = static_cast<HSTORAGE_TYPE>(HvGetCellType(CellIndex));
    ...
    /* Shrink: if old cell is significantly larger, relocate to a smaller cell */
    if (Size < (ULONG)OldCellSize / 2)
    {
        NewCellIndex = HvAllocateCell(RegistryHive, Size, Storage, HCELL_NIL);
        if (NewCellIndex != HCELL_NIL)
        {
            NewCell = HvGetCell(RegistryHive, NewCellIndex);
            RtlCopyMemory(NewCell, OldCell, (SIZE_T)Size);
    ...
    HCELL_INDEX CellIndex)
{
           ...
           reinterpret_cast<ULONG_PTR>(Neighbor) < BwdBinEnd)
    {
        ...
        if (Neighbor->Size > 0)
        {
            ...
            Neighbor = reinterpret_cast<PHCELL>(reinterpret_cast<char*>(Neighbor) + Neighbor->Size);
        }
        else
        {
            Neighbor = reinterpret_cast<PHCELL>(reinterpret_cast<char*>(Neighbor) - Neighbor->Size);
    ...
    IN HCELL_INDEX Cell)
{
    ...
    if (CellRef->Count == CellRef->Max)
    {
        ...
        NewCellArray = static_cast<PHV_HIVE_CELL_PAIR>(CmpAllocate((CellRef->Max + CELL_REF_INCREMENT) * sizeof(HV_HIVE_CELL_PAIR),
                                   TRUE,
                                   TAG_CM));
        if (!NewCellArray)
        {
