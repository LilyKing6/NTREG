/*
 * PROJECT:         NTREG Registry Library
 * LICENSE:         See LICENSE in the top level directory
 * FILE:            cmlib/cmindex.c
                  ...
                  IN PHCELL_INDEX SubKey)
{
    ...
    *SubKey = HCELL_NIL;
        ...
        (Index->Signature == CM_KEY_HASH_LEAF))
    {
        ...
        FastIndex = reinterpret_cast<PCM_KEY_FAST_INDEX>(Index);
        ...
        if (Index->Signature == CM_KEY_FAST_LEAF)
        {
            ...
            CompareLength = mininum(NameLength, ActualNameLength);
                    ...
                    IN PHCELL_INDEX SubKey)
{
    ...
    while (TRUE)
    {
        ...
        Leaf = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, LeafCell));
        if (Leaf)
        {
    ...
    if (Index->Signature == CM_KEY_INDEX_ROOT)
    {
        ...
        for (i = 0; i < Index->Count; i++)
        {
            ...
            if (Number < Leaf->Count)
            {
                ...
                {
                    /* Get the fast index */
                    FastIndex = reinterpret_cast<PCM_KEY_FAST_INDEX>(Leaf);
                      ...
                      IN ULONG Number)
{
    ...
    if (Number < Node->SubKeyCounts[Stable])
    {
        ...
        Index = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, Node->SubKeyLists[Stable]));
    ...
    else if (Hive->StorageTypeCount > Volatile)
    {
        ...
        {
            /* Get the actual key index */
            Index = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, Node->SubKeyLists[Volatile]));
            if (!Index) return HCELL_NIL;
                    ...
                    IN PCUNICODE_STRING SearchName)
{
    ...
    for (i = 0; i < Hive->StorageTypeCount; i++)
    {
        ...
        {
            /* Get the Index */
            IndexRoot = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, Parent->SubKeyLists[i]));
            ...
            if (IndexRoot->Signature == CM_KEY_INDEX_ROOT)
            {
                ...
                IndexRoot = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, SubKey));
            }
            ...
            else
            {
                ...
                SubKey = CmpFindSubKeyByHash(Hive,
                                             reinterpret_cast<PCM_KEY_FAST_INDEX>(IndexRoot),
                                             SearchName);
                  ...
                  IN HCELL_INDEX TargetKey)
{
    ...
    Node = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, TargetKey));
    ...
    if (Node->Flags & KEY_COMP_NAME)
    {
        ...
        SearchName.Buffer = static_cast<PWSTR>(Hive->Allocate(SearchName.Length, TRUE, TAG_CM));
    ...
    Node = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, ParentKey));
    ...
    for (i = 0; i < Hive->StorageTypeCount; i++)
    {
        ...
        if (Node->SubKeyCounts[i])
        {
            ...
            Index = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, IndexCell));
            ...
            if (Index->Signature == CM_KEY_INDEX_ROOT)
            {
                ...
                IndexCell = Child;
                Index = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, Child));
                if (!Index) goto Quickie;
             ...
             IN PCUNICODE_STRING Name)
{
    ...
    else
    {
        ...
        FastLeaf = reinterpret_cast<PCM_KEY_FAST_INDEX>(Leaf);
    ...
    if ((Size / EntrySize) < 1)
    {
        ...
        /* Get the leaf cell */
        Leaf = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, NewCell));
        if (!Leaf)
        {
        ...
        if (FastLeaf) FastLeaf = reinterpret_cast<PCM_KEY_FAST_INDEX>(Leaf);
    }
    ...
    if (FastLeaf)
    {
        ...
        else
        {
            ...
            do
            {
                ...
                if ((USHORT)Name->Buffer[j - 1] > (unsigned char)-1) break;

                /* Otherwise, copy the a character */
                FastLeaf->List[i].NameHint[j - 1] = (unsigned char)Name->Buffer[j - 1];
            } while (--j > 0);
        }
    }
    else
             ...
             IN HSTORAGE_TYPE Type)
{
    ...
    /* Get the cell */
    IndexKey = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, RootCell));
    ...
    LeafKey = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, LeafCell));
    ...
    NewCell = HvAllocateCell(Hive, TotalSize, static_cast<HSTORAGE_TYPE>(Type), LeafCell);
    if (NewCell == HCELL_NIL) return NewCell;

    /* Get the key */
    NewKey = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, NewCell));
    if (!NewKey)
    {
    ...
    {
        /* Copy the fast indexes */
        FastLeaf = reinterpret_cast<PCM_KEY_FAST_INDEX>(LeafKey);
              ...
              IN PHCELL_INDEX *RootCell)
{
    ...
    /* Get the cell */
    IndexKey = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, KeyNode->SubKeyLists[Type]));
             ...
             IN HCELL_INDEX Child)
{
    ...
    /* Get the key node */
    KeyNode = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, Child));
    if (!KeyNode)
    {
        /* Shouldn't happen */
        ASSERT(FALSE);
    ...
    if (KeyNode->Flags & KEY_COMP_NAME)
    {
        ...
        Name.Buffer = static_cast<PWSTR>(Hive->Allocate(Name.Length, TRUE, TAG_CM));
        if (!Name.Buffer)
        {
    ...
    /* Get the parent node */
    KeyNode = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, Parent));
    if (!KeyNode)
    {
        /* Not handled */
        ASSERT(FALSE);
    ...
    if (!KeyNode->SubKeyCounts[Type])
    {
        ...
        IndexCell = HvAllocateCell(Hive, sizeof(CM_KEY_FAST_INDEX), static_cast<HSTORAGE_TYPE>(Type), HCELL_NIL);
    ...
    else
    {
        ...
        Index = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, KeyNode->SubKeyLists[Type]));
        if (!Index)
        {
            /* Not handled */
            ASSERT(FALSE);
            ...
            (Index->Count >= CmpMaxFastIndexPerHblock))
        {
            ...
            /* Convert */
            OldIndex = reinterpret_cast<PCM_KEY_FAST_INDEX>(Index);
                  ...
                  (Index->Count >= CmpMaxIndexPerHblock))
        {
            ...
            IndexCell = HvAllocateCell(Hive,
                                      sizeof(CM_KEY_INDEX) +
                                      sizeof(HCELL_INDEX),
                                      static_cast<HSTORAGE_TYPE>(Type),
                                      HCELL_NIL);
            if (IndexCell == HCELL_NIL)
            {
                /* Not handled */
    ...
    if (Index->Signature == CM_KEY_INDEX_ROOT)
    {
         ...
         */
        LeafCell = CmpSelectLeaf(Hive, KeyNode, &Name, static_cast<HSTORAGE_TYPE>(Type), &RootPointer);
        if (LeafCell == HCELL_NIL)
        {
            /* Not handled */
            ASSERT(FALSE);
                ...
                IN HCELL_INDEX TargetKey)
{
    ...
    else if (LeafIndex < Leaf->Count)
    {
        ...
        else
        {
            ...
            Child = reinterpret_cast<PCM_KEY_FAST_INDEX>(Leaf);
