    ...
    _In_ BITMAP_INDEX MaxLength)
{
    ...
    MaxLength = mininum(MaxLength, BitMapHeader->SizeOfBitMap - StartingIndex);
...
RtlFindMostSignificantBit(ULONGLONG Value)
{
    ...
    {
        return (char)(Position + _BITCOUNT);
    }
...
RtlNumberOfSetBits(
    _In_ PRTL_BITMAP BitMapHeader)
{
    unsigned char *Byte, *MaxByte;
    BITMAP_INDEX BitCount = 0;
    ULONG Shift;

    Byte = (unsigned char*)BitMapHeader->Buffer;
...
retry:
    ...
    {
        /* Retry at the start */
        Margin = mininum(HintIndex + NumberToFind, BitMapHeader->SizeOfBitMap);
        HintIndex = 0;
        goto retry;
    }
    ...
    _Out_ PBITMAP_INDEX StartingRunIndex)
{
    ...
    FromIndex = mininum(FromIndex, BitMapHeader->SizeOfBitMap - 1);
