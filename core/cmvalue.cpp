             ...
             IN HCELL_INDEX Cell)
{
    ...
    /* Get the cell data */
    Value = reinterpret_cast<PCM_KEY_VALUE>(HvGetCell(Hive, Cell));
               ...
               OUT PULONG Length)
{
    ...
    if (!CmpGetValueData(Hive,
                         ...
                         Length,
                         (void**)&Buffer,
                         reinterpret_cast<unsigned char*>(&BufferAllocated),
                         &CellToRelease))
    {
        /* We failed */
             ...
             IN HSTORAGE_TYPE StorageType)
{
    ...
    Value = reinterpret_cast<PCM_KEY_VALUE>(HvGetCell(SourceHive, SourceValueCell));
    if (!Value) ASSERT(FALSE);
    ...
    if (DataSize == 0)
    {
        ...
        NewValue = reinterpret_cast<PCM_KEY_VALUE>(HvGetCell(DestinationHive, NewValueCell));
        ASSERT(NewValue);
