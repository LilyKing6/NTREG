    ...
    _In_ BOOLEAN FixHive)
{
    ...
    KeyNode = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, ParentKey));
    if (!KeyNode)
    {
    ...
    KeyIndex = reinterpret_cast<PCM_KEY_INDEX>(HvGetCell(Hive, KeyNode->SubKeyLists[Stable]));
    if (!KeyIndex)
    {
    ...
    _In_ BOOLEAN FixHive)
{
     ...
     */
    ValueListNode = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, CurrentCell));
    if (!ValueListNode)
    {
