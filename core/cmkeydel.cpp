                ...
                IN BOOLEAN CheckNoSubkeys)
{
    ...
    /* Predefined keys have no normal value list or subkey structure */
    if (CellData->Flags & KEY_PREDEF_HANDLE)
    {
        return TRUE;
    }
                 ...
                 IN BOOLEAN Unlink)
{
    ...
    if (Unlink)
    {
        ...
        ParentData = reinterpret_cast<PCM_KEY_NODE>(HvGetCell(Hive, CellData->Parent));
        ASSERT(ParentData);
    ...
    /* Predefined keys have no normal value/security structure to free */
    if (CellData->Flags & KEY_PREDEF_HANDLE)
    {
