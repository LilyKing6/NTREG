/*
 * PROJECT:     Registry Library
 * PURPOSE:     Type definitions and useful macros (C++20 modernized)
 * COPYRIGHT:   See LICENSE in the top level directory
 */

#ifndef _TYPEDEFS_HOST_H
#define _TYPEDEFS_HOST_H

#include <cassert>
#include <cstdlib>
#include <climits>
#include <cstdint>
#include <cstring>

// Modern C++20 type system
#ifdef __cplusplus
#include "types.hpp"
#include "string.hpp"
#endif

/* GCC的功能属性 */
#if !defined(_MSC_VER) && !defined(__fastcall)
#define __fastcall __attribute__((fastcall))
#endif
#if !defined(_MSC_VER) && !defined(__cdecl)
#define __cdecl __attribute__((cdecl))
#endif
#if !defined(_MSC_VER) && !defined(__stdcall)
#define __stdcall __attribute__((stdcall))
#endif

/* 基本定义 */
#define UNIMPLEMENTED { printf("%s unimplemented\n", __FUNCTION__); exit(1); }
#define UNIMPLEMENTED_ONCE { printf("%s unimplemented\n", __FUNCTION__); exit(1); }
#define ASSERT(x) assert(x)
#define ASSERTMSG(m, x) assert(x)
// #define DPRINT if (0) printf
#define PRINT printf 

#define NDEBUG_ALL 

#ifdef NDEBUG_ALL
    #define DPRINT(args...)
    #define DPRINT1 
    #define CHECKPOINT
#else
    #ifdef NDEBUG
        #define DPRINT(args...)
        #define DPRINT1 
        #define CHECKPOINT
    #else
        #define DPRINT(args...) do { printf("(%s:%d) ",__FILE__,__LINE__); printf(args); } while(0);
        #define DPRINT1 DPRINT
        #define CHECKPOINT do { printf("%s:%d\n",__FILE__,__LINE__); } while(0);
    #endif /* NDEBUG */
#endif

#define IN
#define OUT
#define OPTIONAL

#define FALSE 0
#define TRUE  1

#define ANYSIZE_ARRAY 1

#define OFF_DEFINE

/* 为了排除重复定义 */
#if !defined(_WINDOWS_) // _WINDOWS_

/* Basic types - C++20 modernized with compatibility aliases */
#ifdef __cplusplus
// Use modern types from types.hpp
using UCHAR = registry::u8;
using BYTE = registry::u8;
using USHORT = registry::u16;
using WORD = registry::u16;
using WCHAR = wchar_t;
using UINT = registry::u32;
using ULONG = registry::u32;
using DWORD = registry::u32;
using ULONGLONG = registry::u64;
using UINT64 = registry::u64;

using CHAR = char;
using SHORT = registry::i16;
using INT = registry::i32;
using LONG = registry::i32;
using LONGLONG = registry::i64;
using LONG64 = registry::i64;

using BOOLEAN = bool;
using BOOL = int;
using WINBOOL = int;

// Pointer types
using PSHORT = SHORT*;
using PUSHORT = USHORT*;
using PWORD = WORD*;
using LPWORD = WORD*;
using PWCHAR = WCHAR*;
using PWSTR = WCHAR*;
using LPWSTR = WCHAR*;
using PCWSTR = const WCHAR*;
using LPCWSTR = const WCHAR*;
using PLONG = LONG*;
using LPLONG = LONG*;
using PUINT = UINT*;
using LPUINT = UINT*;
using PULONG = ULONG*;
using PDWORD = DWORD*;
using LPDWORD = DWORD*;
using UINT16 = registry::u16;
using UINT32 = registry::u32;
using ULONG64 = registry::u64;
using DWORD64 = registry::u64;
using PDWORD64 = DWORD64*;
#else
// Legacy C types
typedef char CHAR;
typedef unsigned char UCHAR, BYTE, BOOLEAN;
typedef int16_t SHORT, *PSHORT;
typedef uint16_t USHORT, *PUSHORT, WORD, *PWORD, *LPWORD, WCHAR, *PWCHAR, *PWSTR, *LPWSTR, UINT16;
typedef const uint16_t *PCWSTR, *LPCWSTR;
typedef int32_t INT, LONG, *PLONG, *LPLONG, BOOL, WINBOOL;
typedef uint32_t UINT, *PUINT, *LPUINT, ULONG, *PULONG, DWORD, *PDWORD, *LPDWORD, UINT32;
typedef uint64_t ULONG64, DWORD64, *PDWORD64, UINT64, ULONGLONG;
typedef int64_t LONGLONG, LONG64;
#endif

#if defined(_LP64) || defined(_WIN64)
    typedef int64_t LONG_PTR, *PLONG_PTR, INT_PTR, *PINT_PTR;
    typedef uint64_t ULONG_PTR, DWORD_PTR, *PULONG_PTR, UINT_PTR, *PUINT_PTR;
#else
    typedef int32_t LONG_PTR, *PLONG_PTR, INT_PTR, *PINT_PTR;
    typedef uint32_t ULONG_PTR, DWORD_PTR, *PULONG_PTR, UINT_PTR, *PUINT_PTR;
#endif

typedef uint64_t ULONG64, DWORD64, *PDWORD64, UINT64, ULONGLONG;
typedef int64_t LONGLONG, LONG64;
typedef float FLOAT;
typedef double DOUBLE;

/* 派生类型 */
typedef void* HANDLE;
#ifndef _HAVE_REGHKEY
typedef HANDLE REGHKEY, *PREGHKEY;
#endif

typedef HANDLE HMODULE, HINSTANCE;
typedef INT NTSTATUS, POOL_TYPE;
typedef LONG HRESULT;
typedef ULONG_PTR SIZE_T, *PSIZE_T;
typedef WORD LANGID;

#endif // _WINDOWS_


#if defined(_WINDOWS_)
    #define _HAVE_LIST_ENTRY
    #define _HAVE_LARGE_INTEGER
#endif

#define MAXUSHORT USHRT_MAX



/* 广泛使用的结构 */

#ifndef _HAVE_RTL_BITMAP
typedef struct _RTL_BITMAP
{
    ULONG  SizeOfBitMap;
    PULONG  Buffer;
} RTL_BITMAP, *PRTL_BITMAP;

typedef struct _RTL_BITMAP_RUN
{
    ULONG StartingIndex;
    ULONG NumberOfBits;
} RTL_BITMAP_RUN, *PRTL_BITMAP_RUN;
#endif

#ifndef _HAVE_LARGE_INTEGER
typedef union _LARGE_INTEGER
{
    struct
    {
        ULONG LowPart;
        LONG HighPart;
    };
    struct
    {
        ULONG LowPart;
        LONG HighPart;
    } u;
    LONGLONG QuadPart;
} LARGE_INTEGER, *PLARGE_INTEGER;
#endif

/* 链表节点 */
#ifndef _HAVE_LIST_ENTRY
typedef struct _LIST_ENTRY
{
    // 前向指针
    struct _LIST_ENTRY *Flink;
    // 后向指针
    struct _LIST_ENTRY *Blink;
} LIST_ENTRY, *PLIST_ENTRY;
#endif

#ifndef _HAVE_ANSI_STRING
typedef struct _ANSI_STRING
{
    USHORT Length;
    USHORT MaximumLength;
    char*   Buffer;
} ANSI_STRING, *PANSI_STRING;

typedef struct _UNICODE_STRING
{
    USHORT Length;
    USHORT MaximumLength;
    PWSTR  Buffer;
} UNICODE_STRING, *PUNICODE_STRING;
#endif


/* 链表函数 */
#ifndef _HAVE_LIST_ENTRY
/* 初始化链表头节点，将其Flink和Blink都指向自身 表示链表为空 */
static __inline
void
InitializeListHead(
                   IN PLIST_ENTRY ListHead
                   )
{
    ListHead->Flink = ListHead->Blink = ListHead;
}

/* 在链表的头部插入一个新节点 */
static __inline
void
InsertHeadList(
               IN PLIST_ENTRY ListHead,
               IN PLIST_ENTRY Entry
               )
{
    PLIST_ENTRY OldFlink;
    OldFlink = ListHead->Flink;
    Entry->Flink = OldFlink;
    Entry->Blink = ListHead;
    OldFlink->Blink = Entry;
    ListHead->Flink = Entry;
}

/* 在链表的尾部插入一个新节点 */
static __inline
void
InsertTailList(
               IN PLIST_ENTRY ListHead,
               IN PLIST_ENTRY Entry
               )
{
    PLIST_ENTRY OldBlink;
    OldBlink = ListHead->Blink;
    Entry->Flink = ListHead;
    Entry->Blink = OldBlink;
    OldBlink->Flink = Entry;
    ListHead->Blink = Entry;
}

/* 检查链表是否为空 如果链表头的Flink指向自身，则链表为空 */
static __inline
BOOLEAN
IsListEmpty(
            IN const LIST_ENTRY * ListHead
            )
{
    return (BOOLEAN)(ListHead->Flink == ListHead);
}

/* 删除链表中的一个节点 同时会修复前后节点的指针，保持链表的完整性 */
static __inline
BOOLEAN
RemoveEntryList(
                IN PLIST_ENTRY Entry)
{
    PLIST_ENTRY OldFlink;
    PLIST_ENTRY OldBlink;

    OldFlink = Entry->Flink;
    OldBlink = Entry->Blink;
    OldFlink->Blink = OldBlink;
    OldBlink->Flink = OldFlink;
    return (BOOLEAN)(OldFlink == OldBlink);
}

/* 移除链表的第一个节点，并返回被移除的节点。 */
static __inline
PLIST_ENTRY
RemoveHeadList(
               IN PLIST_ENTRY ListHead)
{
    PLIST_ENTRY Flink;
    PLIST_ENTRY Entry;

    Entry = ListHead->Flink;
    Flink = Entry->Flink;
    ListHead->Flink = Flink;
    Flink->Blink = ListHead;
    return Entry;
}

/* 移除链表的最后一个节点，并返回被移除的节点 */
static __inline
PLIST_ENTRY
RemoveTailList(
               IN PLIST_ENTRY ListHead)
{
    PLIST_ENTRY Blink;
    PLIST_ENTRY Entry;

    Entry = ListHead->Blink;
    Blink = Entry->Blink;
    ListHead->Blink = Blink;
    Blink->Flink = ListHead;
    return Entry;
}
#endif

#ifndef _HAVE_ANSI_STRING
typedef const UNICODE_STRING *PCUNICODE_STRING;
#endif

#undef LOBYTE
#undef HIBYTE
#undef LOWORD
#undef HIWORD
#undef MAKEWORD
#undef MAKELONG
/* 广泛使用的宏 */
#define LOBYTE(w)               ((BYTE)(w))
#define HIBYTE(w)               ((BYTE)(((WORD)(w)>>8)&0xFF))
#define LOWORD(l)               ((WORD)((DWORD_PTR)(l)))
#define HIWORD(l)               ((WORD)(((DWORD_PTR)(l)>>16)&0xFFFF))
#define MAKEWORD(a,b)           ((WORD)(((BYTE)(a))|(((WORD)((BYTE)(b)))<<8)))
#define MAKELONG(a,b)           ((LONG)(((WORD)(a))|(((DWORD)((WORD)(b)))<<16)))

#define MAXULONG 0xFFFFFFFF

#define NT_SUCCESS(x)           ((x)>=0)
#undef FIELD_OFFSET
#if !defined(__GNUC__)
#define FIELD_OFFSET(t,f)       ((LONG)(LONG_PTR)&(((t*) 0)->f))
#else
#define FIELD_OFFSET(t,f)       ((LONG)__builtin_offsetof(t,f))
#endif

#undef RTL_CONSTANT_STRING
#define RTL_CONSTANT_STRING(s)  { sizeof(s)-sizeof((s)[0]), sizeof(s), s }

#undef CONTAINING_RECORD
#define CONTAINING_RECORD(address, type, field)  ((type *)(((ULONG_PTR)address) - (ULONG_PTR)(&(((type *)0)->field))))

#undef RtlZeroMemory
#undef RtlCopyMemory
#undef RtlMoveMemory
#define RtlZeroMemory(Destination, Length)            memset(Destination, 0, Length)
#define RtlCopyMemory(Destination, Source, Length)    memcpy(Destination, Source, Length)
#define RtlMoveMemory(Destination, Source, Length)    memmove(Destination, Source, Length)

#undef MAKELANGID
#undef PRIMARYLANGID
#undef SUBLANGID
#undef SUBLANG_NEUTRAL
#define MAKELANGID(p,s)         ((((WORD)(s))<<10)|(WORD)(p))
#define PRIMARYLANGID(l)        ((WORD)(l)&0x3ff)
#define SUBLANGID(l)            ((WORD)(l)>>10)
#define SUBLANG_NEUTRAL         0x00

/* 防止包含其他标头 */
#define __INTERNAL_DEBUG
#define RTL_H

#endif

